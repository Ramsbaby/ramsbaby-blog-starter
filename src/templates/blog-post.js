...
const BlogPostTemplate = ({ data, pageContext, location }) => {
  ...
  const tableOfContents = (
    <ul className="post-single__table_of_contents-list" role="list">
      {post &&
  ...
  // Precompute Open Graph image URL (Helmet does not accept React.Fragment children)
  const ogFileName =
    ((pageContext.slug || '')
      .replace(/^\/+|\/+$/g, '')
      .replace(/\/+|\s+/g, '_') || 'home') + '.png'
  const ogUrl = `${siteUrl}/og/${ogFileName}`
  ...
  return (
    <Layout location={location} title={title} siteMetadata={metaData}>
      ...
      >
        ...
        <nav
          className="post-single__table_of_contents"
